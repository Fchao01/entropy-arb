...
class AsterVenue:
    ...
    def _apply_partial_depth(self, event):
        bids = event.get("b") or event.get("bids") or []
        asks = event.get("a") or event.get("asks") or []
        self.book.apply_hl([
            [{"px": p, "sz": q} for p, q in bids],
            [{"px": p, "sz": q} for p, q in asks],
        ])

    async def _book_loop(self, stop, notify):
        """Maintain a top-20 Aster book from the partial-depth stream."""
        retry_sec = 1.0
        while not stop.is_set():
            try:
                stream = f"{self.ws_url}/ws/{self.symbol.lower()}@depth20@100ms"
                async with ws_connect(stream, ping_interval=20, ping_timeout=20, max_size=2**23) as ws:
                    async for raw in ws:
                        e = __import__('json').loads(raw)
                        self._apply_partial_depth(e)
                        notify()
                        retry_sec = 1.0
                ...
                wait = retry_sec
                log.warning("[ASTER] websocket failed; retrying in %.0fs: %s",
                            wait, e)
